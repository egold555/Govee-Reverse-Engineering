"""H6061 Glide Hexa — 20-byte BLE frames (plaintext)."""
...
def seg_color(mask: int, r: int, g: int, b: int) -> bytes:
    p = _blank()
...
def seg_brightness(mask: int, level: int) -> bytes:
    p = _blank()
...
def panel_color(panel: int, r: int, g: int, b: int) -> bytes:
    return seg_color(PANEL_MASK[panel], r, g, b)


def panel_brightness(panel: int, level: int) -> bytes:
    return seg_brightness(PANEL_MASK[panel], level)
...
def parse_a5(pkt: bytes):
    """Return (page, [(pct, r, g, b)] * 4) or None."""
