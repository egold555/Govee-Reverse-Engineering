#!/usr/bin/env python3
"""Paint or read H6088 cubes over BLE. Force-quit the Govee app first.

  python cli.py frames
  python cli.py scan
  python cli.py paint --name Govee_H6088 --cube 1 --rgb ff0000 --dim 100
  python cli.py status --name Govee_H6088
"""
...
def cmd_frames(_args):
    ...
    for n in range(1, 7):
        print(f"cube {n} red  {hex20(cube_n_color(n, 255, 0, 0))}")
...
async def cmd_paint(args):
    ...
    async with client:
        ...
        if args.all:
            for n in CUBE_BIT:
                await write20(client, cube_n_color(n, r, g, b))
        else:
            await write20(client, cube_n_color(args.cube, r, g, b))
...
async def cmd_status(args):
    ...
    async with client:
        ...
        for page in (1, 2):
            ...
            colors.extend(slots)
        for i, (pct, r, g, b) in enumerate(colors[:6], start=1):
            print(f"cube {i}  {pct:3d}%  #{r:02x}{g:02x}{b:02x}")
    return 0


def main():
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--name", default="Govee_H6088", help="ADV name prefix")
    sub = p.add_subparsers(dest="cmd", required=True)
    sub.add_parser("frames")
    sub.add_parser("scan")
    paint = sub.add_parser("paint")
    paint.add_argument("--cube", type=int, default=1, choices=range(1, 7))
    paint.add_argument("--all", action="store_true")
